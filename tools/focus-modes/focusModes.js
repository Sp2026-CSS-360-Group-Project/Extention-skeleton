...
const CRUD_STORAGE_KEY = "focusKit:focusModes";
...
const DEFAULT_FOCUS_MODES = FOCUS_MODES;
...
// Check if a focus mode id is valid against the built-in registry.
function isValidFocusMode(modeId) {
  return FOCUS_MODES.some(mode => mode.id === modeId);
}

// Determine if a focus mode should apply tab muting.
...
function loadFocusModes(callback) {
  chrome.storage.local.get([CRUD_STORAGE_KEY], (data) => {
    const stored = data[CRUD_STORAGE_KEY];
    if (!Array.isArray(stored)) {
      chrome.storage.local.set({ [CRUD_STORAGE_KEY]: DEFAULT_FOCUS_MODES }, () => {
...
function saveFocusModes(modes, callback) {
  chrome.storage.local.set({ [CRUD_STORAGE_KEY]: modes }, () => {
...
function updateFocusMode(modeId, changes, callback) {
  loadFocusModes((modes) => {
    const updated = modes.map((mode) => {
      ...
      return { ...mode, ...changes, id: mode.id, builtIn: mode.builtIn };
    });
  ...
  loadFocusModes((modes) => {
    ...
    chrome.storage.local.get([ACTIVE_MODE_STORAGE_KEY], (data) => {
      ...
      if (data[ACTIVE_MODE_STORAGE_KEY] === modeId) {
        ops.push(new Promise((res) => chrome.storage.local.remove(ACTIVE_MODE_STORAGE_KEY, res)));
      }
...
if (typeof globalThis !== "undefined") {
  ...
  globalThis.FocusKitFocusModes = globalThis.FocusKitModes;
}
